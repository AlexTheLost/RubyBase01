...
if __FILE__ == $0
    if ARGV.length != 3
        puts "Error: should be three arguments (county:'BY|RU'US', nubmer percon:'1..1000000', probability:'0..1')"
        exit
    end
    checkFirstArg(ARGV[0])
    checkSecondArg(ARGV[1])
    checkThirdArg(ARGV[2])
    addr = Address.new()
    addr.setParameters(ARGV[0].to_s, ARGV[1].to_i, ARGV[2].to_i)
    addr.createDataSet()
end
